...
const authMiddleware = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (authHeader) {
        try {
            ...
            const idIsPresent = yield user_1.default.findByPk(decoded.id).then((user) => user).catch((error) => { logger_1.default.error(error); return res.status(401).json({ error: 'Error on our server. Try later' }); });
            if (idIsPresent)
                return next();
            else
